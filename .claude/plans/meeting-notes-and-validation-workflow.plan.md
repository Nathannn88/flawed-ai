...
> 状态：历史计划，已实施；其中路径、8 对旧规则和 Obsidian 工作流已被 2026-07-30 的 repo-first 结构替代
>
> **当前入口**：最高设计权威为 `design/00-项目核心/最高设计权威.md`，活动导航为 `design/设计资料索引.md`。本文中的旧路径仅记录当时执行环境，不是当前依赖，不据此恢复已删除的 `.base`、`*MAP.md` 或旧分板块规则。

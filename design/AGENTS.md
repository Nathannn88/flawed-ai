...
术语以主旨第十一节"术语约定"为准（诗人 / 神像（诗人的化身）/ 宠物 / 萨满五子 / 信仰值 / 智慧能量）。
...
| 子项目 | 路径 | 职责 |
|-------|------|------|
| 世界观与叙事 | `story/` | 项目哲学、Art 岛诗人传说、4+1 岛屿、三阶段结构、社区互访愿景 |
...
> 但一直未启用，磁盘上无此目录。待 v3 编码阶段确有需要时再建，当前忽略。
...
3. Claude：读变更 → 审慎追问/挑战 → 检查与主旨一致 → 协助细化 → 更新文稿 + `记录/进度.md`（重大决策另记 `记录/决策与活动日志.md`）

> 当前阶段到此为止（设计协作）。把设计落到代码（原 `/apply-design` 流程，已删，待 v3 编码时重建）属于**未来 v3 编码阶段**，见根 `CLAUDE.md` 冻结附录。
...
`v1-archive/` 保存 v1 旧版设计（海难诗人 + 荒岛→出海 + 灯塔），仅供参考，不影响当前 v3 开发。
